  ...
  methods: {
    ...
    onValueChanged(prop, val) {
      ...
      this.reportList.map(item => {
        ...
        Income.OperatingMarginSuggest = (Income.OperationRevenue + Income.MainBusinessIncome + Income.OperatingCosts + Income.BusinessTariffAndAnnex + Income.ConsumptionTax + Income.BusinessTax + Income.UrbanMaintenanceAndConstructionTax + Income.ResourcesTax + Income.LandValueIncrementTax + Income.LandUse_House_VehVesl_StmpTax + Income.EduSurtax_MineralRsrcCompFees_SwgChrg).toFixed(2)
        Income.OperatingProfitSuggest = (Income.SellingExpenses + Income.CostOfGoodsMaintenance + Income.AdRate_BusinessPropagandizeFee + Income.AdministrationExpenseCost + Income.OrganizationCosts + Income.BusinessEntertainment + Income.ResearchExpenditure + Income.FinancialCost + Income.InterestCost + Income.AssetsImpairmentLoss + Income.ExplorationExpenditure + Income.OtherExpenses + Income.IncomeFromInvestment + Income.NetIncomeFromChangesInFairValue + Income.ExchangeEarning).toFixed(2)
        Income.TotalProfitSuggest = (Income.NonOperatingIncome + Income.PublicSubsidy + Income.NonBusinessExpenditure + Income.LossOnBadDebts + Income.OtherProfit + Income.UnrecoverableLossOnLongTermBondInvestment + Income.UnrecoverableLossOnLongTermEquityInvestment + Income. LossByForceMajeureFactors + Income.TaxDelayCharge + Income.TotalTax).toFixed(2)
