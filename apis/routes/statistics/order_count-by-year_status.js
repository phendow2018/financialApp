...
class order_CountByYearStatus extends Base{
  constructor(ctx){
      super(ctx);
...
order_CountByYearStatus.prototype.doRead = async function(){
    let _this = this;
    let postData  = _this.ctx.request.body;
    let queryData = _this.ctx.query;

    let deal = new OrderStatistics();
    let ret = await deal.getCountGroupByYearStatus();
    if (ret === false) {
...
order_CountByYearStatus.prototype.read = async function(){
    let _this = this;
...
router.all('/',async (ctx, next) => {
    let instance = await new order_CountByYearStatus(ctx);
    await instance.deal();
})
