...
OrderCancel.prototype.check = async function(postData) {
...
OrderCancel.prototype.doCreate = async function() {
  let _this = this;
...
OrderCancel.prototype.qichachaSend = async function(postData) {
  try {
    let qichachaReport = new QichachaReport();
    let ret = await qichachaReport.makeCancel(postData.OrderNumber);
    if (ret === false) {
    ...
    fs.writeFileSync(`${__dirname}/../../log/cancel.json`, jsonStr);
...
OrderCancel.prototype.create = async function(){
    let _this = this;
...
router.all('/',async (ctx, next) => {
    let instance = await new OrderCancel(ctx);
    await instance.deal();
})
